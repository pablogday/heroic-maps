...
- [ ] **#8 `.h3m` parser** — Standalone parser sub-project at `src/lib/h3m/`. Auto-fill upload form, backfill empty `victory_condition` / `loss_condition` for 2,958 scraped maps. Designed to be extractable to its own npm package later.
  ...
  - [x] **`.h3c` campaign archives** — 53 files in the corpus. New `src/lib/h3c/` parser cracks the campaign descriptor (header + scenario list). Backfilled `maps.campaign_data` JSONB on all 53; map detail page now shows a `<CampaignBlock>` with numbered scenarios, region color, difficulty, and prologue/epilogue snippets. MapCard sprouts a "Campaign" badge. Coverage: 81% of campaigns parse fully (43/53). Remaining 8 are HotA formatVersion=3 (HotA 1.8.0+) which has a 12-byte fixed prefix not yet decoded; 2 are scenario-block read errors. Embedded scenario .h3m maps (subsequent gzip members) not yet extracted — future work.
...
- **Path-1 collapse: Favorite → Bookmark** — two private "save for later" booleans were doing the same job. Migration 0011 merged favorited rows into bookmarked then dropped the column. UI surfaces collapsed (MapCard footer, MapActions sidebar, Library tabs, profile section, copy). Public-favorites-with-counts (Path 2) parked as a follow-up if ranking-relevant signals become useful.
- **Icons unified around game-icons.net** — every chrome surface (nav, user dropdown, mobile drawer, MapCard buttons, sidebar Specs/Origin/Conditions, Library tabs, EmptyState glyphs) uses the game-icons silhouette set with `currentColor` theming. Quill/scroll empty-state with HoMM3 flavor copy.
- **Faction crests: real HoMM3 town banners (PNG) by default** — 48×32 pixel-art town portraits via `FactionCrestPixel`; legacy hand-drawn `FactionCrestSvg` lives behind `NEXT_PUBLIC_FACTION_CREST_STYLE` feature flag for switching back. Towns sidebar grid is 5×2 on desktop, 3-across on narrow viewports.
- **Map Contents card** — per-row icons + a tooltip on the heading that links to the parser source on GitHub (open-source signal).
- **Conditions card** — per-type icons for victory (laurel/chalice/cyclops/etc.) + loss (skull-bones/stone-tower/sands-of-time/etc.).
